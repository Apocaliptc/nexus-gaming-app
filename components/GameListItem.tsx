...
export const GameListItem: React.FC<Props> = ({ game, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className="bg-nexus-800/40 p-3 md:p-4 rounded-xl md:rounded-2xl border border-nexus-800 flex items-center gap-3 md:gap-4 hover:bg-nexus-800/80 hover:border-nexus-accent/50 transition-all cursor-pointer group shadow-lg"
    >
      ...
      <div className="flex-1 min-w-0">
         <div className="flex justify-between items-start">
            <h4 className="text-xs md:text-base font-bold text-white truncate pr-2 group-hover:text-nexus-accent transition-colors">{game.title}</h4>
            <ChevronRight className="text-nexus-800 md:group-hover:text-white transition-all shrink-0" size={14} md:size={16} />
         </div>

         <div className="flex items-center gap-3 md:gap-4 text-[9px] md:text-[10px] text-gray-500 mt-1">
            <span className="flex items-center gap-1 font-bold text-gray-400">
               ...
               {game.achievementCount}/{game.totalAchievements}
            </span>
         ...
         <div className="mt-2 md:mt-3 flex items-center gap-2 md:gap-3">
            ...
            <span className={`text-[8px] md:text-[9px] font-black w-6 text-right ${percent === 100 ? 'text-nexus-accent' : 'text-gray-400'}`}>{percent}%</span>
         </div>
      </div>
